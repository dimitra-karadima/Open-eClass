...
if(isset($forumcatnotify)) { // modify forum category notification
		$rows = mysql_num_rows(db_query("SELECT * FROM forum_notify
			WHERE (user_id = '".mysql_real_escape_string($_GET['$uid'])."') AND (cat_id = '".mysql_real_escape_string($_GET['$cat_id'])."') AND ( course_id = '".mysql_real_escape_string($_GET['$cours_id'])."')", $mysqlMainDb));
		if ($rows > 0) {
			db_query("UPDATE forum_notify SET notify_sent = '".mysql_real_escape_string($_GET['$forumcatnotify'])."'
				WHERE (user_id = '".mysql_real_escape_string($_GET['$uid'])."') AND (cat_id = '".mysql_real_escape_string($_GET['$cat_id'])."') AND (course_id ='".mysql_real_escape_string($_GET['$cours_id'])."')", $mysqlMainDb);
	} else {
		db_query("INSERT INTO forum_notify SET user_id = '".mysql_real_escape_string($_GET['$uid'])."',
		cat_id = '".mysql_real_escape_string($_GET['$cat_id'])."', notify_sent = 1, course_id = '".mysql_real_escape_string($_GET['$cours_id'])."'", $mysqlMainDb);
	}
} elseif(isset($forumnotify)) { // modify forum notification
	$rows = mysql_num_rows(db_query("SELECT * FROM forum_notify
		WHERE (user_id = '".mysql_real_escape_string($_GET['$uid'])."') AND (forum_id = '".mysql_real_escape_string($_GET['$forum_id'])."') AND (course_id = '".mysql_real_escape_string($_GET['$cours_id'])."')", $mysqlMainDb));
	if ($rows > 0) {
		db_query("UPDATE forum_notify SET notify_sent = '".mysql_real_escape_string($_GET['$forumnotify'])."'
			WHERE (user_id = '".mysql_real_escape_string($_GET['$uid'])."') AND (forum_id = '".mysql_real_escape_string($_GET['$forum_id'])."') AND (course_id = '".mysql_real_escape_string($_GET['$cours_id'])."')", $mysqlMainDb);
	} else {
		db_query("INSERT INTO forum_notify SET user_id = '".mysql_real_escape_string($_GET['$uid'])."',
		forum_id =  '".mysql_real_escape_string($_GET['$forum_id'])."', notify_sent = 1, course_id = '".mysql_real_escape_string($_GET['$cours_id'])."'", $mysqlMainDb);
	}
}
...
if ($total_categories) {
	...
	$limit_forums = "";
	if ($viewcat != -1) {
		$limit_forums = "WHERE f.cat_id = '".mysql_real_escape_string($_GET[$viewcat])."'";
	}
			...
			if ($forum_row[$x]["cat_id"] == $categories[$i]["cat_id"]) {
				...
				if ($is_adminOfCourse) { // admin //
					$sqlTutor = db_query("SELECT id FROM student_group
						WHERE (forumId= '".mysql_real_escape_string($_GET['$forum'])."') AND (tutor='".mysql_real_escape_string($_GET['$uid'])."')", $currentCourseID );
